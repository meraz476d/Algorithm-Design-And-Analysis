...
int main(){
    ...
    while (leftI <= rightI){                  // Checks if leftI is less than or equal to rightI
        int mid = (leftI + rightI) / 2;       // Calculating mid index

        if (arr[mid] == k){        // Checks if the value of mid index is equal to k
            f = true;              // If value of mid index is equal to k, the value of f will be true
            break;                 // breaking the loop
        }else if (arr[mid] > k){      // Checks if the value of mid is greater than k
            rightI = mid - 1;         // The right index is the previous index of mid
        } else{                    // The value of mid is less than the k
